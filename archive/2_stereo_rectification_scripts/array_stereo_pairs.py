...
ap.add_argument("-i", "--infile", required=True, type=str,
	help="filename for input file containing comma delimited string of numbers of frames to capture")
...
#Open text file of comma delimited string
text_file = open(infile, "r")

#read whole file to a string
file_contents = text_file.read()
text_file.close()
frame_list = file_contents.split(',')
frame_list = map(int, frame_list)	#make integers
frame_list = sorted(frame_list)		#sort the list
...
while(cap.isOpened()):
    ...
    theframe = start + frametext
    if any(x == int(theframe) for x in frame_list): #compare current frame to input list, if matches, write frames
        print("Writing frame " + str(theframe))
        cv2.imwrite(dir_path + "CHECKER_L_" + str(theframe) + ".png", adjusted)
        cv2.imwrite(dir_path + "CHECKER_R_" + str(theframe) + ".png", adjusted2)
        i += 1
