...
# Now use this height to adjust the cY value correctly
frametext = 0
    ...
    for c in contours:
        if cv2.contourArea(c) > args["minArea"]:
            ...
            if M["m00"] != 0:
                ...
                writefile.write(f"{frametext}\t{cX}\t{cY_flipped}\t{cv2.contourArea(c)}\t{min_val}\t{max_val}\t{mean_val[0]}\n")
                cv2.drawContours(adjusted, [c], -1, (255, 180, 10), -1)

    if args["look"] == 1:
        if cX is not None and cY is not None:
            cv2.putText(adjusted, f"{frametext},X:{cX}", (30, frame_height - 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 180, 10))
