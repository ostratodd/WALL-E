...
for index, row in sorted.iterrows():
    #first form pulses if contiguous frames are close in both x and y. Index is sorted by camera, so does each camera separately
    if index > 0:
        difX = abs(int(row['cX']) - sorted.iloc[index-1]['cX']) 	#difference in x-value for sorted rows
	...
	#here, merge into a pulse if x and y are close enough or
        ...
        if (difF == 1 and oldDifF ==0) or (difX < XMAX and difY < YMAX and difF < FS) or (difF == 0): 
            if len(curpulse.index) > 0 :	#add to exising pulse
                #print("\tAdding to existing pulse")
        ...
        else :	#pulse ended so add new column to give the pulse a name below
...
pulses = pulses.sort_values(['pulse', 'camera', 'cY', 'cX', 'frame'])
